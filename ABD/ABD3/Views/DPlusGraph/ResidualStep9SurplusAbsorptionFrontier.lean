...
normal form from Step 8, while excluding the easy support-small,
surplus-isolated, and unit-boundary frontiers.  This is the precise place where
the next real number-theoretic lemma must act. -/
...
small-support, surplus-isolated, and unit-boundary frontiers, the radical side
must actually be large. -/
...
frontier once support-small, surplus-isolated, and unit-boundary have been
excluded. -/
theorem step9Frontier_of_radSmall_noAccepted (T : ABCData) (P : PowerData)
    ...
    (hnotIsolated : ¬ T.ResidualSurplusIsolated P G R)
...
/-- Outside the graph-exceptional side, the surplus-isolated residual frontier is
impossible. -/
theorem not_residualSurplusIsolated_of_not_dPlusGraphExceptional
    (T : ABCData) (P : PowerData)
    (G : ArithmeticSyncGenerators) (R : ℕ)
    (hnotExceptional : ¬ T.DPlusGraphExceptional P) :
    ¬ T.ResidualSurplusIsolated P G R := by
  intro hisolated
  unfold ResidualSurplusIsolated at hisolated
  exact hnotExceptional
    (T.dPlusGraphExceptional_of_residualFrontier P G R
      (T.residualFrontier_of_surplusIsolated P G R hisolated))

/-- A non-graph-exceptional triple has neither support-small, surplus-isolated,
nor unit-boundary, so radical-small plus no accepted edge enters the Step 9
frontier. -/
theorem surplusAbsorptionFrontier_of_radicalSmall_of_noAccepted_of_notGraphExceptional
  ...
  exact T.step9Frontier_of_radSmall_noAccepted
    ...
    (T.not_residualSurplusIsolated_of_not_dPlusGraphExceptional P G R hnotExceptional)
    (by
      intro hunit
