...
import ABD.ABD.Lattice.AdditiveLinearFormLinear
...
/-- Additive kernels are closed under subtraction of tangent vectors. -/
theorem sub_mem_additiveKernel
    {S : Finset ℕ} {a b c : ℕ} {x y : Tangent S}
    (hx : x ∈ AdditiveKernel S a b c)
    (hy : y ∈ AdditiveKernel S a b c) :
    x - y ∈ AdditiveKernel S a b c := by
...
/-- Triple additive kernels are closed under subtraction of tangent vectors. -/
theorem sub_mem_tripleAdditiveKernel
    {T : ABCTriple} {x y : Tangent T.support}
    (hx : x ∈ TripleAdditiveKernel T)
    (hy : y ∈ TripleAdditiveKernel T) :
    x - y ∈ TripleAdditiveKernel T := by
  exact sub_mem_additiveKernel
    (S := T.support) (a := T.a) (b := T.b) (c := T.c) hx hy
